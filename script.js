...
(function () {
  ...
  /* ===================================================
     4. FADE IN / OUT ANIMATIONS
     Step 1: Add js-ready to body — this activates the
     CSS that hides .fade-el elements. Without this class,
     ALL content is visible (safe fallback).
     Step 2: Observer watches elements and adds .visible
     as they enter the viewport.
  ==================================================== */

  // IMMEDIATELY signal JS is running — enables the fade animation
  ...
  // Collect ALL .fade-el elements (including the ones just added)
  ...
  // Immediately show anything already in the viewport on page load
  ...
  // VISIBILITY FIX: Immediately show any fade-el already in the viewport on load
  // This prevents text from being stuck invisible if the observer fires late.
  setTimeout(function () {
    allFadeEls.forEach(function (el) {
      var rect = el.getBoundingClientRect();
  ...
  // SAFETY NET: After 2.5 seconds, force-show any element still invisible.
  // Protects against slow connections, cached pages, or any observer edge case.
  setTimeout(function () {
    allFadeEls.forEach(function (el) {
      el.classList.add('visible');
    });
